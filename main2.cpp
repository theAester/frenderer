...
int main(int argc, char* argv[]){
	...
	camera cam(vec3<double>(-7, 0, 0), vec3<double>(0,0,0), 72.6, 1); 
	scene sc(XRES,YRES,cam,vec3<double>(0.1,0.1,0.1));

	donut* don = new donut(vec3<double>(0,0,0), vec3<double>(M_PI/2,0,0), 0.6,3,true,3.5,3.5,1.4);
	donut* don2 = new donut(vec3<double>(0,0,0), vec3<double>(0,0,0), 0.3,1.3,true,1.5,1.5,0.5);
